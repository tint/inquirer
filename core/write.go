...
func WriteAnswer(t interface{}, name string, v interface{}) (err error) {
	...
	case reflect.Struct:
		...
		// copy the value over to the normal struct
		return copyReflect(field, value)
	case reflect.Map:
...
// Usage:
//
//	if err := inquirer.Ask(qs, &v); err != nil {
...
func copyReflect(t reflect.Value, v reflect.Value) (err error) {
	...
	if isList(v) && isList(t) {
		...
		for i := 0; i < v.Len(); i++ {
			...
			case reflect.Slice:
				...
				if err := copyReflect(obj, v.Index(i)); err != nil {
					return err
				}
			...
			case reflect.Array:
				...
				if err := copyReflect(t.Slice(i, i+1).Index(0), v.Index(i)); err != nil {
					return err
				}
			}
		}
