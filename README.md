...
[![GoDoc](http://img.shields.io/badge/godoc-reference-5272B4.svg)](https://pkg.go.dev/github.com/tint/inquirer)

This package started as a copy of go-survey/survey but has since been modified to fit inquirer's specific needs.
...
var qs = []*inquirer.Question{
    ...
    {
        Name: "age",
        Prompt:   &inquirer.Input{Message: "How old are you?"},
    },
}
...
func main() {
    ...
    answers := struct {
        Name          string                    // inquirer will match the question and field names
        FavoriteColor string `inquirer:"color"` // or you can tag fields to match a specific name
        Age           int                       // if the types don't match, inquirer will convert it
    }{}

    // perform the questions
    err := inquirer.Ask(qs, &answers)
    if err != nil {
...
possible to change inquirer's default behaviors by passing `AskOpts` to either `Ask` or `AskOne`. Examples
...
inquirer.AskOne(prompt, &color, inquirer.WithValidator(inquirer.Required))
...
inquirer.Ask(questions, &answers, inquirer.WithValidator(inquirer.Required))
```
...
```golang
text := ""
prompt := &inquirer.Multiline{
    Message: "ping",
}
inquirer.AskOne(prompt, &text)
```
...
```golang
password := ""
prompt := &inquirer.Password{
    Message: "Please type your password",
}
inquirer.AskOne(prompt, &password)
```
...
prompt := &inquirer.MultiSelect{..., PageSize: 10}

// or as an option to Ask or AskOne
inquirer.AskOne(prompt, &days, inquirer.WithPageSize(10))
```
...
```golang
days := []string{}
prompt := &inquirer.MultiSelect{
    Message: "What days do you prefer:",
    Options: []string{"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"},
}
inquirer.AskOne(prompt, &days)
```
...
inquirer.AskOne(prompt, &content)
```
...
inquirer.AskOne(prompt, &color, inquirer.WithFilter(myFilter))
```
...
// or define a default for all of the questions
inquirer.AskOne(prompt, &color, inquirer.WithKeepFilter(true))
```

## Validation

Validating individual responses for a particular question can be done by defining a
`Validate` field on the `inquirer.Question` to be validated. This function takes an
`interface{}` type and returns an error to show to the user, prompting them for another
response. Like usual, validators can be provided directly to the prompt or with `inquirer.WithValidator`:

```golang
q := &inquirer.Question{
    Prompt: &inquirer.Input{Message: "Hello world validation"},
...
color := ""
prompt := &inquirer.Input{ Message: "Whats your name?" }

// you can pass multiple validators here and inquirer will make sure each one passes
inquirer.AskOne(prompt, &color, inquirer.WithValidator(inquirer.Required))
```

### Built-in Validators

`inquirer` comes prepackaged with a few validators to fit common situations. Currently these
validators include:
...
inquirer.AskOne(prompt, &number, inquirer.WithRemoveSelectAll())
```
...
inquirer.AskOne(prompt, &number, inquirer.WithRemoveSelectNone())
```


### Changing the input rune

In some situations, `?` is a perfectly valid response. To handle this, you can change the rune that inquirer
...
inquirer.AskOne(prompt, &number, inquirer.WithHelpInput('^'))
```
...
inquirer.AskOne(prompt, &number, inquirer.WithIcons(func(icons *inquirer.IconSet) {
...
inquirer will assign prompt answers to your custom types if they implement this interface:
...
if you are manipulating the cursor or using `inquirer`, you will need a way to interpret terminal / ANSI escape sequences
...
### What kinds of IO are supported by `inquirer`?

inquirer aims to support most terminal emulators; it expects support for ANSI escape sequences.
This means that reading from piped stdin or writing to piped stdout is **not supported**,
and likely to break your application in these situations. See [#337](https://github.com/AlecAivazis/inquirer/pull/337#issue-581351617)
...
When Survey reads a ^C byte (ASCII \x03, "end of text"), it interrupts the current inquirer and returns a
`github.com/tint/inquirer/terminal.InterruptErr` from `Ask` or `AskOne`.
If you want to stop the process, handle the returned error in your code:

```go
err := inquirer.AskOne(prompt, &myVar)
if err != nil {
